"""
    ...
    Run by calling run() (see run() description)
...
class Scraper:
    ...
    def __init__(self):
        ...
        self.headers_user = {"User-Agent": "Mozilla/5.0"}


    def get_sreality(self, category_type, category_main = 1, locality_region = 10):
        ...
        total = requests.get(get_estates(1), headers = self.headers_user).json()["pagination"]["total"]

        print(f"Found {total} listings")

        try:
            r = requests.get(get_estates(total+1), headers = self.headers_user)
    ...
    def get_description(self, code):
        ...
        try:
            r = requests.get(base_url, headers=self.headers_user)
        except requests.exceptions.RequestException as e:
            print(e)
            return None
    ...
    def convert_description_to_df(self, json_data):
        db = pd.DataFrame()
    ...
    def get_all_sreality(self, category_type: int):


        json_data = self.get_sreality(category_type)
    ...
    def get_all_description(self, db):
        list_of_dfs = []

        def get_details_of_codes(codes):
            iteration = 1
            for code in list(codes):
                json_data = self.get_description(code)
                if len(json_data) == 1:
                    continue  # Invalid code
                if iteration > 1:
                    print("\r", end="")
                if iteration == self.limit_for_details+1:
                    print("")
                print(f"Getting details ({iteration})", end="")
                list_of_dfs.append(self.convert_description_to_df(json_data))
                iteration += 1


        if db["code"].shape[0] > self.limit_for_details:
            required_runs = round((db["code"].shape[0] - self.limit_for_details)/self.limit_for_details)
            print(f"Sequential scraping required as there are too many details missing. Run me {required_runs} more times")
            get_details_of_codes(list(db["code"])[:self.limit_for_details])
    ...
    def parse(self, base, desc):
        db = pd.merge(base, desc, on='code')
        db = db.apply(self.clean, axis=1)
    ...
    def run(self, intent: str):
        ...
        def get_street_city(db):
            ...
            start = meta.find(f"k {get_intent[self.intention[intent]]}") + len(f"k {get_intent[self.intention[intent]]}")
            end = meta.find(";")
        ...
        if master_db is None:
            ...
            if len(new_ids) != 0:
                new_ids_desc,req_runs = self.get_all_description(new_ids)
                new_ids_complete = self.parse(new_ids, new_ids_desc)
            ...
            def mark_if_deleted(db):
                db["time_of_deletion"] = str(self.date)
